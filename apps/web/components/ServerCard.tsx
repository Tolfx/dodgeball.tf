...
export default function ServerCard({ server }: Props) {
  ...
  const fetchAdditionInfo = async () => {
    ...
    const res = await fetch(
      `${
        isPROD ? "https://api.dodgeball.tf" : "http://localhost:3001"
      }/servers/serverinfo/${ip}/${port}`
    );
        ...
        <CardContent>
          ...
          <Typography component={"span"}>
            {!additionInfo ? (
              ...
              <>
                <Typography variant="h5">Map: {additionInfo.map}</Typography>
              </>
            )}
          </Typography>
          <Typography component={"span"}>
            {!additionInfo ? (
              ...
              <>
                <Typography component={"span"} variant="h5">
                  Players: {additionInfo?.players?.length}/
                  {additionInfo?.maxplayers}
                  {/* @ts-ignore */}
                  <ExpandMore
                    expand={expanded}
