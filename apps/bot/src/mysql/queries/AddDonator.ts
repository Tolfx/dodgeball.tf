...
export default function AddDonator(donator: DonatorUser, serverAmounts = 3)
{
  ...
  const superRandompassword = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
  ...
  const queryCreateAdmin = `INSERT INTO sourcebans.sb_admins (user,authid,password,gid,email,validate,extraflags,immunity,srv_group,srv_flags,srv_password,lastvisit) VALUES ('Donator | ${donator.steamName}', '${(new SteamID(donator.steamId).steam2())}', '${superRandompassword}', ${getGroupId(donator)}, '${donator.steamName}@dodgeball.tf', NULL, 0, 0, '${getGroupName(donator)}', NULL, NULL, NULL);`;


  return (connection: mysql.Connection): Promise<Array<any>> => new Promise((resolve, reject) =>
  {
    connection
      .query(queryCreateAdmin,
        (err, results) =>
        {
          if (err)
            return reject(err);
          ...
          {
            serverValues.push(`(${adminId}, ${getGroupId(donator)}, -1, ${i + 1})`);
          }
          ...
          connection
            .query(queryCreateAdminServerGroups,
              (err, results) =>
              {
                if (err)
                  return reject(err);
