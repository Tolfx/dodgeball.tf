...
export default function GetTopSpeedPlayers(
  serverid?: string,
  steamid?: string
) {
  ...
  const serveridQuery = serverid
    ? `${!steamid ? "WHERE " : ""}serverid = '${serverid}'`
    : "";
  const steamidQuery = steamid
    ? `${serverid ? "AND " : "WHERE "}steamid = '${steamid}'`
    : "";

  const query = `SELECT * FROM topspeed.topspeed ${serveridQuery} ${steamidQuery} ORDER BY topspeed DESC`;

  return (connection: mysql.Connection): Promise<Array<TopSpeedPlayer>> =>
