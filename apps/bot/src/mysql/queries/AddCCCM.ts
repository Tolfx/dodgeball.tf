...
export default function AddCCCM(donator: DonatorUser) {
  ...
  const queryCheckCCCM = `SELECT * FROM cccm.cccm_users WHERE auth = '${new SteamID(
    donator.steamId
  ).steam2()}';`;

  return (connection: mysql.Connection): Promise<boolean> =>
    new Promise((resolve, reject) => {
      connection.query(queryCheckCCCM, (err, results) => {
        ...
        const queryCreateCCCM = `INSERT INTO cccm.cccm_users (auth, hidetag, tagcolor, namecolor, chatcolor, tag) VALUES ('${new SteamID(
          donator.steamId
        ).steam2()}', 0, '${cccm.tagcolor}', '', '', '${cccm.tag}');`;

        connection.query(queryCreateCCCM, (err) => {
          if (err) return reject(err);
