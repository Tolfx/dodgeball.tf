  ...
  return (connection: mysql.Connection): Promise<Array<any>> =>
    new Promise((resolve, reject) => {
      connection.query(queryGetDonator, (err, results) => {
        ...
        const queryUpateDonator = `UPDATE sourcebans.sb_admins SET gid = ${getGroupId(
          donator
        )}, srv_group = '${getGroupName(
          donator
        )}' WHERE authid = '${new SteamID(donator.steamId).steam2()}';`;

        connection.query(queryUpateDonator, (err) => {
          if (err) return reject(err);
          ...
          const queryUpdateAdminServerGroups = `UPDATE sourcebans.sb_admins_servers_groups SET group_id = ${getGroupId(
            donator
          )} WHERE admin_id = ${adminId};`;

          console.log(queryUpdateAdminServerGroups);

          connection.query(queryUpdateAdminServerGroups, (err, results) => {
            if (err) return reject(err);
