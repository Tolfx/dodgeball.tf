...
export default function UpdateCCCM(options: UpdateCCCMOptions) {
  ...
  // we also need to ensure that we don't add a comma at the end of the query if we don't have any data to update

  let queryUpdateCCCM = `UPDATE cccm.cccm_users SET ${
    options.tag ? `tag = '${options.tag}', ` : ""
  }${options.chatcolor ? `chatcolor = '${options.chatcolor}', ` : ""}${
    options.namecolor ? `namecolor = '${options.namecolor}', ` : ""
  }${
    options.tagcolor ? `tagcolor = '${options.tagcolor}', ` : ""
  }WHERE auth = '${new SteamID(options.steamid).steam2()}';`;
  ...
  return (connection: mysql.Connection): Promise<Array<any>> =>
    new Promise((resolve, reject) => {
      connection.query(queryUpdateCCCM, (err, results) => {
