...
export default class LinkInteraction implements InteractionsHandler
{
  ...
  async command(interaction: ChatInputCommandInteraction)
  {
    ...
    const isLinked = await LinkedAccountModel.findOne({ discordId: interaction.user.id });

    if (isLinked)
      return interaction.reply({ content: 'You already have a linked account', ephemeral: true });

    const embed = new EmbedBuilder()
      .setTitle('Link your account')
      .setDescription('Link your account to your discord account to get access to more features.')
      .addFields([
        {
