...
export default class InspectPlayerInteractions implements InteractionsHandler
{
  ...
  async command(interaction: ChatInputCommandInteraction)
  {
    ...
    const [uniqueId] = await GetPlayerUniqueId(uSteamid)(this.services.getMysqlConnection());

    if (!uniqueId)
      return interaction.reply({ content: 'Player not found', ephemeral: true });

    const [player] = await GetPlayer(String(uniqueId.playerId))(this.services.getMysqlConnection());
