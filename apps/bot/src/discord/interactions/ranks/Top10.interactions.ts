...
export default class Top10Interactions implements InteractionsHandler
{
  ...
  async command(interaction: ChatInputCommandInteraction)
  {
    ...
    const serverid = interaction.options.data.find((option) => option.name === 'server')?.value;
    if (!serverid) return;
    const top10 = await GetTopPlayers(String(serverid))(this.services.getMysqlConnection());
    const embed = new EmbedBuilder()
      .addFields(top10.map((player, index) => ({
