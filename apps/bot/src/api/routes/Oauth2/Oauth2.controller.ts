...
export default class Oauth2Controller implements ControllerRouter {
  ...
  public async linkAccounts(req: Request, res: Response) {
    if (!req.session.discord_token) return res.redirect("/oauth2/discord");
    ...
    if (!req.session.steam_id) return res.redirect("/oauth2/steam");
    ...
    this.client?.guilds.cache
      ...
      ?.members.add(await this.client.users?.fetch(discordInfo.id), {
  ...
  public async steamCallback(req: Request, res: Response) {
    const [steamuser, eSteamuser] = await AsyncAwait<any>(
  ...
  public discord(req: Request, res: Response) {
    ...
    const discordUri = `https://discord.com/oauth2/authorize?client_id=${DISCORD_BOT_ID}&redirect_uri=${encodeURIComponent(
      callbackUrl
  ...
  public async discordCallback(req: Request, res: Response) {
    const [auth, eAuth] = await AsyncAwait<any>(
