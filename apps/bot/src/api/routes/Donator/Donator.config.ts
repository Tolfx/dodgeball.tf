  ...
  constructor(public server: Application, public services: Services)
  {
    ...
    this.router.get('/', async (req, res) =>
    {
      ...
      if (!req.isAuthenticated())
        return res.redirect('/donator/auth/steam');
      res.send(DonateTemplate(parseFloat(String(query.amount ?? 2.5))));
    });
    this.router.get('/auth/steam', passport.authenticate('steam', { failureRedirect: '/donator' }));
    this.router.get('/auth/steam/callback', passport.authenticate('steam', { failureRedirect: '/auth/steam' }), (req, res) =>
    {
      res.redirect('/donator');
    });

    this.router.get('/stripe', this.controller.startStripePayment.bind(this.controller));
